...
import com.example.cupcake.model.ZERO_VALUE

/**
 * [CustomerFragment] allows the user to choose a pickup date for the cupcake order.
 */
class CustomerFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding?.apply {
            ...
            customerFragment = this@CustomerFragment
     ...
     */
    fun goToNextScreen() {
        findNavController().navigate(R.id.action_customerFragment_to_summaryFragment)
    }
    ...
    fun setErrorMessage(text: CharSequence) {
        if (text.length == ZERO_VALUE) {
            binding?.tilEnterName?.error = getString(R.string.enter_name_error)
            binding?.nextButton?.isEnabled = false

        } else {
            binding?.tilEnterName?.error = ""
            binding?.nextButton?.isEnabled = true
            sharedViewModel.setName(text.toString())
        }
    }
}